...
// Send #len bytes
// ----------------------------------------------------------------------------
void SendUart2 (uint8_t *array, char len) {
    do{
        SendChar2(*(array++));
    }while(--len);
}
...
// ----------------------------------------------------------------------------
void GetString2 (uint8_t *stream, uint8_t *streamlength) {
    struct buf_st *p = &rbuf;
    int index=0;
    *streamlength=0;
    ...
    if (SIO_RBUFLEN != 0){
        ...
        switch(p->buf [(p->out) & (RBUF_SIZE - 1)]){
            
            case 0x72: // r=read register
                *streamlength=(p->buf [((p->out) & (RBUF_SIZE - 1))+1]);
                if(*streamlength > SIO_RBUFLEN)   {*streamlength=SIO_RBUFLEN;}
                
                while(SIO_RBUFLEN>0){
                    *(stream+(index++)) = (p->buf [(p->out++) & (RBUF_SIZE - 1)]);
                }
            ...
            default:
                *streamlength=SIO_RBUFLEN;
                while(SIO_RBUFLEN>0){
                    *(stream+(index++)) = (p->buf [(p->out++) & (RBUF_SIZE - 1)]);
                }
                //*streamlength=1;
                //*stream = (p->buf [(p->out++) & (RBUF_SIZE - 1)]);
            break;
